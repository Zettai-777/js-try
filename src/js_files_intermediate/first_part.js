...
/*
    ...
    через вложенные callback-и. Создавая асинхронную функцию, если есть понимание, что внутри будет использваться другая
    асинхронная функция, можно создать promise через new Promise((resolve, reject) => {}), внутри которого будет выполнена
    некоторая логика переданным экзекютером, а далее вызвать метод .then(). Для связи нескольких асинхронных функций внутри
    метода необходимо вовзращать новый promise. Но также можно возвращать обычные объекты (например какие то данные, переданные
    через входной параметр в then(). Так же в promise-ах существует механизм обработки ошибок в асинхронных функциях. В таком
    случае вместо resolve - удачного выполнения, передаётся reject - неудачное выполнение запроса, ошибку из которого можно
    отловить при помощи catch(). Помимо вышеперечисленного после then() - catch() может идти блок finally(), который будет
    исполнен в любом случае вне зависимоти от успеха или неудачи запроса. (короче try-catch-finally из Java)
    Глобальные методы класса Promise:
    - all([]) - принимает массив promise-ов и выполняет дальнейшие действия только после выполнения всех переданных promise-ов
    - race([]) - принимает массив promise-ов и выполняет дальнейшие действия после выполнения быстрейшего promise-а
 */
...
//         setTimeout(()=> {
//             data.modified = true;
//             // reject(data); //неудачное событие
//             resolve(data);  //успешное событие
//         }, 2000);
//     })
// }).then(clientData => {
//     console.log('Data received', clientData);
...
// sleep(ms = prompt('Введите количество милисекунд:'))
//     .then(() =>
//         console.log(`After ${ms/1000} seconds`));

// Глобальные функции класса Promise
